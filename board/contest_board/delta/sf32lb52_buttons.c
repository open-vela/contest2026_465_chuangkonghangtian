...
#define SF32LB52_1_PIN      GET_PIN_2(hwp_gpio1, 34)
#define SF32LB52_0_PIN      GET_PIN_2(hwp_gpio1, 11)
#define SF32LB52_1_ACTIVE_LOW 0
#define SF32LB52_0_ACTIVE_LOW 0
...
{
  btn_buttonset_t mask = 0;
  bool level1 = sifli_gpio_read(SF32LB52_1_PIN);
  bool level2 = sifli_gpio_read(SF32LB52_0_PIN);

#if SF32LB52_1_ACTIVE_LOW
  if (!level1) mask |= (1 << 1);
#else
  if (level1)  mask |= (1 << 1);
#endif

#if SF32LB52_0_ACTIVE_LOW
  if (!level2) mask |= (1 << 0);
#else
  if (level2)  mask |= (1 << 0);
#endif
                                ...
                                FAR void *arg)
{
  ...
  if (either == 0 || handler == NULL)
    {
      if (priv->irq_enabled)
        {
          sifli_gpio_set_event(SF32LB52_0_PIN, false, false,
                               NULL, NULL);
  ...
  if (!priv->irq_enabled)
    {
      ret = sifli_gpio_set_event(SF32LB52_1_PIN, true, true,
                                 sf32lb52_button_interrupt, priv);
      if (ret >= 0)
        {
          ret = sifli_gpio_set_event(SF32LB52_0_PIN, true, true,
                                     sf32lb52_button_interrupt, priv);
        }
      if (ret >= 0)
        {
...
int sf32lb52_button_initialize(FAR const char *devname)
{
  ...
  sifli_gpio_config(SF32LB52_1_PIN, GPIO_INPUT);
  sifli_gpio_config(SF32LB52_0_PIN, GPIO_INPUT);
